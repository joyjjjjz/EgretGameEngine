...
import { Swf } from "../Swf";
import { StarlingSwfUtils } from "./StarlingSwfUtils";
import { StarlingSwfMovieClip } from "./StarlingSwfMovieClip";
import { SwfAssetManager } from "../SwfAssetManager";
/**
 * Created by yangsong on 2014/6/16.
 * StarlingSwf工厂类
 */
export class StarlingSwfFactory extends SingtonClass {
    private swfAssetsManager: SwfAssetManager;
    private swfAssetsNames: Array<string>;
    private swfAssets: Array<Swf>;
    private swfData: any;
    /**
     * 构造函数
     */
    public constructor() {
        super();
        this.swfAssetsManager = new SwfAssetManager();
        this.swfAssetsNames = new Array<string>();
        this.swfAssets = new Array<Swf>();
        this.swfData = {};
    }
    /**
     * 添加一个swf
    ...
    public addSwf(name: string, swfData: Object, spriteSheep: egret.SpriteSheet): void {
        ...
        var swf: Swf = new Swf(swfData, this.swfAssetsManager, 24);
        swf.name = name;
    ...
    public stopSwfs(arr: Array<string>): void {
        for (var i: number = 0, len: number = arr.length; i < len; i++) {
            var swf: Swf = StarlingSwfUtils.getSwf(arr[i]);
            if (swf) {
