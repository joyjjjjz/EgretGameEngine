import { SwfMovieClip } from "../display/SwfMovieClip";
import { SwfSprite } from "../display/SwfSprite";
import { Swf } from "../Swf";
/**
 * Created by lcj on 14-6-18.
 * StarlingSwf工具类
 */
export class StarlingSwfUtils {
    private static swfList: Array<Swf> = [];
    /**
     * 函数游戏内用到的swf
     * @param swf
     */
    public static addSwf(swf: Swf): void {
     ...
     * @param swf
     */
    public static removeSwf(swf: Swf): void {
     ...
     * @returns {*}
     */
    public static createSprite(name: string, data: any[] = null, sprData: any[] = null): SwfSprite {
        var l: number = StarlingSwfUtils.swfList.length;
        for (var i: number = 0; i < l; i++) {
            var swf: Swf = StarlingSwfUtils.swfList[i];
     ...
     * @returns {*}
     */
    public static createMovie(name: string, data: any[] = null, cls: any = null): SwfMovieClip {
        var l: number = StarlingSwfUtils.swfList.length;
        for (var i: number = 0; i < l; i++) {
            var swf: Swf = StarlingSwfUtils.swfList[i];
     ...
     * @returns {*}
     */
    public static getSwf(name: string): Swf {
        var l: number = StarlingSwfUtils.swfList.length;
        for (var i: number = 0; i < l; i++) {
            var swf: Swf = StarlingSwfUtils.swfList[i];
            if (swf.name == name) {
     ...
     * @param thisObj
     */
    public static fixButton(btn: SwfMovieClip, onClick?: Function, thisObj?: any): void {
    ...
    private static onBtnTouchBegin(event: egret.TouchEvent): void {
        var btn: SwfMovieClip = <SwfMovieClip>event.currentTarget;
...
export class StarlingSwfButtonData {
    public btn: SwfMovieClip;
